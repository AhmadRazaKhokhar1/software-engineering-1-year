...
**Q1.** Which `position` value removes an element from the normal flow AND locks it to the viewport?
...
**Q2.** What must a parent have for `position: absolute` on a child to work relative to it?

- A) `display: flex`
- B) `position: relative` (or another position value) ✅
- C) `overflow: hidden`
- D) `z-index: 1`

---

**Q3.** Which Flexbox property controls the direction items are laid out?
...
**Q4.** How do you perfectly center an element both horizontally and vertically with Flexbox?

- A) `display: flex; text-align: center; vertical-align: middle;`
- B) `display: flex; justify-content: center; align-items: center;` ✅
- C) `display: flex; align-content: center;`
- D) `display: flex; margin: auto;`

---

**Q5.** What does `justify-content: space-between` do?

- A) Centers all items
- B) Puts equal space around each item
- C) First item at start, last at end, equal space between ✅
- D) All items stack at the end

---

**Q6.** What does `flex: 1` do on a flex item?

- A) Makes the item invisible
- B) Sets the item's order to 1
- C) Makes the item grow to fill available space equally ✅
- D) Sets the item's width to 1px

---

**Q7.** Which position value creates a hybrid between `relative` and `fixed`?
...
**Q8.** What does the `gap` property do in Flexbox?

- A) Creates space between flex items ✅
- B) Creates padding inside flex items
- C) Creates a border around flex items
- D) Removes space between flex items

---

**Q9.** What does `z-index` control?

- A) The horizontal position of an element
- B) The stacking order (which element appears on top) ✅
- C) The zoom level of an element
- D) The opacity of an element

---

**Q10.** In a sidebar layout, what flex value keeps the sidebar at a fixed 300px width?
...
**Q1. Explain the difference between `position: absolute` and `position: fixed`. Give one use case for each.**

> **`position: absolute`** removes an element from the normal document flow and positions it relative to its **nearest positioned ancestor** (an ancestor with `position: relative`, `absolute`, `fixed`, or `sticky`). The element scrolls with the page.
> - **Use case:** A notification badge positioned on the top-right corner of an avatar image.
>
> **`position: fixed`** removes an element from the normal flow and positions it relative to the **browser viewport**. The element stays in place even when the user scrolls.
> - **Use case:** A fixed navigation bar at the top of the page that remains visible while scrolling.
>
> **Key difference:** Absolute elements scroll with their parent; fixed elements never scroll — they're locked to the screen.

---

**Q2. What are the two axes in Flexbox? How do `justify-content` and `align-items` relate to them?**

> Flexbox has two axes:
> 1. **Main Axis** — The primary direction items flow (horizontal by default with `flex-direction: row`, vertical with `column`)
> 2. **Cross Axis** — The axis perpendicular to the main axis
>
> `justify-content` controls distribution along the **main axis** (e.g., `center`, `space-between`).
>
> `align-items` controls alignment along the **cross axis** (e.g., `center`, `flex-start`, `stretch`).
>
> ```css
...
**Q3. Write the CSS for a navigation bar with a logo on the left and menu links on the right, vertically centered, with a dark background.**
...
> `space-between` pushes the logo and links to opposite ends. `align-items: center` vertically centers both. A nested `display: flex` on `.nav-links` makes the links sit side by side with `gap` for spacing.

---

**Q4. Explain the `position: sticky` behavior. How is it different from `fixed`?**

> `position: sticky` is a **hybrid** of `relative` and `fixed`:
> - Initially, the element behaves like `position: relative` (stays in the normal document flow)
> - When the user scrolls past a defined threshold (e.g., `top: 0`), the element "sticks" and behaves like `position: fixed`
> - The element only sticks **within its parent container** — once the parent scrolls out of view, the sticky element scrolls away too
>
> **Difference from `fixed`:**
> - `fixed` is ALWAYS locked to the viewport, regardless of parent
> - `sticky` only sticks within its parent's boundaries
> - `fixed` elements are removed from the flow; `sticky` elements remain in the flow until they stick
>
> ```css
> .sticky-header {
>     position: sticky;
...
**Q5. Create a Flexbox card layout where 3 cards sit side by side with equal width, 20px gap, and wrap to the next line on smaller screens. Each card should have a minimum width of 280px.**
...
> - `flex: 1` makes all cards grow equally to fill available space
> - `min-width: 280px` prevents cards from becoming too narrow
> - `flex-wrap: wrap` allows cards to wrap to the next line when the container is too narrow to fit all cards at their minimum width
> - `gap: 20px` creates consistent spacing without margin issues
>
> On a wide screen: 3 cards side by side. On a narrow screen: cards wrap to 2 per row, then 1 per row.
