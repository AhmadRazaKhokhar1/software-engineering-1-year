...
**Q1.** What does `e.preventDefault()` do in a form submit handler?

- a) Prevents JavaScript from running
- b) Stops the form from being validated
- c) Stops the browser from reloading the page ✅
- d) Removes all event listeners

---

**Q2.** Which attribute disables the browser's built-in form validation?
...
**Q3.** What event fires on every keystroke in an input field?
...
**Q4.** What does `aria-live="polite"` do?

- a) Makes the element invisible
- b) Tells screen readers to announce changes when the user is idle ✅
- c) Prevents the element from being focused
- d) Adds animation

---

**Q5.** Which is the best way to show form errors?

- a) `alert("Error!")`
- b) `console.log("Error")`
- c) Inline text next to the field with clear message ✅
- d) Change the page background to red

---

**Q6.** What does `new FormData(form)` do?

- a) Resets the form
- b) Validates all fields
- c) Collects all form field names and values ✅
- d) Sends the form to a server

---

**Q7.** When does the `blur` event fire?

- a) When the user clicks a button
- b) When the user starts typing
- c) When the user clicks away from a field ✅
- d) When the page loads

---

**Q8.** Why should you add `<label>` elements to form inputs?

- a) They make the form look bigger
- b) They are required by JavaScript
- c) They improve accessibility and allow clicking the label to focus the input ✅
- d) They prevent form submission

---

**Q9.** What does `aria-invalid="true"` tell screen readers?

- a) The field is disabled
- b) The field is hidden
- c) The field has an error ✅
- d) The field is optional

---

**Q10.** What is the purpose of the `novalidate` attribute on a form?

- a) To remove all input fields
- b) To let JavaScript handle validation instead of the browser ✅
- c) To make the form read-only
...
**Q1.** Why do we use `e.preventDefault()` in form submission? What happens without it?

> **Answer:** `e.preventDefault()` stops the browser's default behavior of reloading the page and sending data to a server URL. Without it, the page refreshes, all JavaScript state is lost, and the user experience breaks in single-page applications.

---

**Q2.** What is the difference between HTML built-in validation and custom JavaScript validation? When should you use each?

> **Answer:** HTML validation uses attributes like `required`, `minlength`, `pattern` — it's quick but gives limited control over error messages and styling. Custom JS validation gives full control over when, where, and how errors appear. Use HTML validation for simple forms; use JS validation when you need branded error messages, real-time feedback, or complex rules.

---

**Q3.** What are ARIA attributes and why are they important in forms?

> **Answer:** ARIA (Accessible Rich Internet Applications) attributes provide extra information to assistive technologies like screen readers. `aria-required` tells the user a field is mandatory, `aria-invalid` signals an error, and `aria-describedby` links an input to its error message. They make forms usable for visually impaired users.

---

**Q4.** Explain the difference between `input`, `change`, and `blur` events on a form field.

> **Answer:** `input` fires on every keystroke as the user types. `change` fires only when the user changes the value AND leaves the field (for text inputs) or immediately for checkboxes/selects. `blur` fires when the user clicks away from the field regardless of whether the value changed. Use `input` for real-time feedback, `blur` for validation after the user finishes a field.

---

**Q5.** Why is it important to focus the first invalid field after a failed form submission?

> **Answer:** Focusing the first invalid field guides the user directly to where the problem is, saving them from scanning the entire form. It's especially important for accessibility — screen reader users would otherwise not know which field has an error. It creates a smoother, more professional user experience.
