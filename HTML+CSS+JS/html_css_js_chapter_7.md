...
- b) Makes it return a Promise automatically ✅
- c) Makes it run twice
- d) Makes it synchronous

---

**Q2.** What happens when JavaScript encounters `await`?

- a) It stops the entire browser
- b) It pauses that function until the Promise resolves ✅
- c) It skips to the next function
...
**Q4.** Why do we need `response.json()` after fetch?

- a) To validate the response
- b) To convert the raw response body into a JavaScript object ✅
- c) To send data back to the server
- d) To check for errors

---

**Q5.** When does `fetch()` reject its Promise?

- a) On 404 errors
- b) On 500 errors
- c) On network failures (no internet, DNS errors) ✅
- d) On any non-200 status

---

**Q6.** What is `Promise.all()` used for?

- a) Running promises one after another
- b) Cancelling all promises
- c) Running multiple promises in parallel and waiting for all ✅
- d) Retrying failed promises

---

**Q7.** What does `try/catch` do in an async function?

- a) Prevents the function from running
- b) Catches errors from rejected promises or thrown errors ✅
- c) Makes the function synchronous
- d) Retries the code automatically
...
**Q9.** Which is the correct way to handle errors with fetch?

- a) `fetch(url).error(callback)`
- b) `fetch(url).then().catch()` ✅
- c) `fetch(url).onError(callback)`
- d) `fetch(url).reject(callback)`

---

**Q10.** What is a loading state and why is it important?

- a) A JavaScript variable type
- b) Visual feedback shown while data is being fetched ✅
- c) A CSS animation property
- d) A way to speed up API calls

---

## 9 — Short Answer Questions

**Q1.** Explain the difference between synchronous and asynchronous code with an example.

> **Answer:** Synchronous code runs **line by line** — each line waits for the previous one to finish. Asynchronous code starts a task and **moves on** without waiting. Example: `console.log("A"); setTimeout(() => console.log("B"), 1000); console.log("C");` outputs A, C, B — because setTimeout is async and doesn't block the next line.

---

**Q2.** What is a Promise? Describe its three states.

> **Answer:** A Promise is an object representing the eventual completion or failure of an async operation. Its three states are: **Pending** (initial state, still waiting), **Fulfilled/Resolved** (operation completed successfully, `.then()` runs), and **Rejected** (operation failed, `.catch()` runs). Once settled (fulfilled or rejected), a Promise cannot change state.

---

**Q3.** Why does `fetch()` need two steps to get usable data? (`response` → `response.json()`)

> **Answer:** The first `await fetch()` gives you a **Response object** containing metadata (status, headers) but the body hasn't been fully read yet. `response.json()` is a second async operation that **reads and parses the body stream** into a JavaScript object. They're separate because you might want to check `response.ok` or `response.status` before bothering to parse the body.

---

**Q4.** What is the Event Loop and why does JavaScript need it?

> **Answer:** The Event Loop is a mechanism that allows JavaScript to perform non-blocking operations despite being single-threaded. It continuously checks if the Call Stack is empty, and if so, moves callbacks from the Callback Queue to the Call Stack. This lets JavaScript handle timers, API calls, and user events without freezing the page.

---

**Q5.** Why should every API call in your app handle three states (loading, success, error)?

> **Answer:** **Loading** — gives users visual feedback that something is happening, preventing confusion or repeated clicks. **Success** — displays the fetched data properly. **Error** — tells users what went wrong and often provides a retry option. Without all three states, the app feels broken — users won't know if it's loading, and if something fails, they'll see a blank screen with no explanation.
