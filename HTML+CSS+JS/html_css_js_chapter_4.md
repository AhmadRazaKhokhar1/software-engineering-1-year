...
**Q1.** What is the BEST way to change an element's appearance with JavaScript?

- A) Set inline styles with `element.style.property`
- B) Toggle CSS classes with `element.classList.toggle()` ✅
- C) Use `innerHTML` to change the element
- D) Remove and recreate the element

---

**Q2.** How do you update a CSS custom property (variable) with JavaScript?

- A) `document.style.setProperty("--color", "red")`
- B) `document.documentElement.style.setProperty("--color", "red")` ✅
- C) `document.css.setVariable("--color", "red")`
- D) `document.root.style["--color"] = "red"`

---

**Q3.** Which event fires AFTER a CSS transition finishes?

- A) `transitionstart`
- B) `transitionend` ✅
- C) `animationend`
- D) `transitioncomplete`

---

**Q4.** What does `window.matchMedia("(max-width: 768px)")` return?

- A) A boolean
- B) A MediaQueryList object with a `.matches` property ✅
- C) The current window width
- D) A CSS string

---

**Q5.** Where should theme colors be defined for a theme switcher?

- A) In JavaScript variables
- B) In CSS custom properties (variables) ✅
- C) In HTML data attributes
- D) In inline styles

---

**Q6.** What is `IntersectionObserver` used for?

- A) Observing CSS changes
- B) Detecting when elements enter or leave the viewport ✅
- C) Watching for DOM mutations
- D) Monitoring network requests

---

**Q7.** How do you persist a user's theme preference across page visits?
...
**Q8.** What does `requestAnimationFrame()` do?

- A) Pauses the animation
- B) Runs a callback before the next screen repaint for smooth animation ✅
- C) Creates a CSS animation
- D) Sets a timeout of 16ms

---

**Q9.** Which CSS property should be used for the TRANSITION when toggling between light and dark themes?

- A) `animation`
- B) `transition: background-color 0.4s ease, color 0.4s ease` ✅
- C) `transform`
- D) `opacity`

---

**Q10.** What is the correct architecture for a theme switcher?

- A) Define all colors in JavaScript
- B) CSS variables for colors + JS toggles a class + localStorage saves preference ✅
- C) Inline styles changed by JavaScript
- D) Two separate CSS files loaded dynamically

---

## 📝 5 Short Answer Questions

**Q1. Explain the "CSS defines, JS decides" principle. Why is it considered best practice?**

> The principle means **CSS should define all possible visual states** (active, hidden, dark-theme, error, etc.) while **JavaScript only decides WHEN to apply them** by toggling classes.
>
> ```css
> /* CSS DEFINES the visual state */
> .modal { opacity: 0; visibility: hidden; transition: all 0.3s ease; }
> .modal.active { opacity: 1; visibility: visible; }
> ```
> ```javascript
> // JS DECIDES when to apply it
> modal.classList.toggle("active");
> ```
>
> **Why it's best practice:**
> 1. **Separation of concerns** — styling stays in CSS, behavior in JS
> 2. **CSS transitions work automatically** — no JavaScript animation code needed
> 3. **Easier maintenance** — designers change CSS, developers change JS
> 4. **Better performance** — browsers optimize CSS transitions with GPU acceleration
> 5. **Reusability** — the same class works on any element

---

**Q2. Describe three different methods to control CSS with JavaScript. When would you use each?**

> 1. **Class Toggling** — `element.classList.add("active")` — Use for predefined visual state changes like active/inactive, open/closed, light/dark. This is the default best practice for most situations.
>
> 2. **Inline Styles** — `element.style.width = "75%"` — Use for dynamic, calculated values that can't be predefined in CSS — like positions based on mouse coordinates, sizes based on data values, or user-selected colors from a color picker.
>
> 3. **CSS Custom Properties** — `document.documentElement.style.setProperty("--primary", "#e74c3c")` — Use for theme-wide changes where one variable affects many elements, like switching entire color schemes, adjusting global font sizes, or implementing user-customizable themes.
>
> **Rule of thumb:** Start with class toggling. Use inline styles for runtime-calculated values. Use CSS variables for global theme changes.

---

**Q3. How does a complete dark mode theme switcher work? Describe the architecture from CSS to JavaScript to persistence.**

> A complete theme switcher works in four layers:
>
> **1. CSS Variables:** Define two sets of colors — one in `:root` (light theme) and one in `html.dark-theme` (dark theme). All styles use `var(--variable-name)`.
>
> **2. CSS Transitions:** Add `transition: background-color 0.4s ease, color 0.4s ease` to elements for smooth theme switching.
>
> **3. JavaScript Toggle:** Add/remove the `dark-theme` class on the `<html>` element. One class change switches ALL colors because CSS variables cascade down.
>
> **4. Persistence:** Save the user's choice in `localStorage`. On page load, check `localStorage` first, then fall back to `window.matchMedia("(prefers-color-scheme: dark)")` for system preference.
>
> ```javascript
> // On page load: apply saved or system preference
...
**Q4. What is `IntersectionObserver` and why is it better than using scroll event listeners for animations?**

> `IntersectionObserver` is a browser API that **asynchronously detects when elements enter or leave the viewport** (or any specified container). You create an observer, tell it which elements to watch, and it calls your callback when they become visible.
>
> ```javascript
> const observer = new IntersectionObserver((entries) => {
>     entries.forEach(entry => {
...
> ```
>
> **Why it's better than scroll listeners:**
> 1. **Performance** — scroll events fire hundreds of times per second, causing jank. IntersectionObserver runs asynchronously off the main thread
> 2. **No calculations** — no need to manually calculate element positions with `getBoundingClientRect()`
> 3. **Battery friendly** — especially important on mobile devices
> 4. **Clean API** — simple callback pattern instead of complex scroll math
> 5. **Built-in threshold** — easily control how much of the element must be visible

---

**Q5. Write JavaScript code that creates a notification system — a function that accepts a message and type (success/error/info), creates a notification element, animates it in, and auto-removes it after 3 seconds.**
...
```
> 
> The corresponding CSS uses `transform: translateX(120%)` as the default state and `transform: translateX(0)` for the `.show` class, with a `transition` for smooth animation. The `requestAnimationFrame` ensures the browser registers the initial state before adding the `.show` class, triggering the animation. The `transitionend` event ensures the element is removed from the DOM only after the exit animation completes.
